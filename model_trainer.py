...
def get_args():
    ...
    parser.add_argument('--wf_cohesion', type=float, default=0.0, help='weight factor for [modular] loss cohesion')
    parser.add_argument('--wf_coupling', type=float, default=0.0, help='weight factor for [modular] loss coupling')
...
def train_one_epoch(model, train_loader, modular_params, optimizer, tqdm_desc=""):
    ...
    wf_loss_cohesion = modular_params["wf_cohesion"]
    wf_loss_coupling = modular_params["wf_coupling"]
    wf_loss_compactness = modular_params["wf_compactness"]
    modular_model_training_mode = any([wf_loss_cohesion,
                                       wf_loss_coupling,
                                       wf_loss_compactness])
    ...
    for i, (images, labels) in enumerate(pbar):
        ...
        if modular_model_training_mode:
            ...
            curr_loss = curr_loss_ce + \
                        (wf_loss_cohesion * curr_loss_cohesion +
                         wf_loss_coupling * curr_loss_coupling +
    ...
    if modular_model_training_mode:
        loss_value_dict.update({"loss_cohesion": mean_list(all_loss_cohesion),
...
def build_related_dirs(args, run_version="", skip_if_checkpoint_exists=True):
    ...
    if "wf_cohesion" in args.__dict__:
        model_checkpoint_name += f"__coh{args.wf_cohesion}_coup{args.wf_coupling}_comp{args.wf_compactness}"
