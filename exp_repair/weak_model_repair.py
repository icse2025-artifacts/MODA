...
from dataset_loader import supported_repair_datasets, load_repair_dataset, load_dataset
...
def main():
    ...
    # weak_model__cm = evaluate_model_in_confusion_matrix(weak_model, test_loader, device=DEVICE, num_classes=num_classes)
    # repaired_model__cm = evaluate_model_in_confusion_matrix(repaired_model, test_loader, device=DEVICE,
    #                                                         num_classes=num_classes)
    # plot_confusion_matrix(weak_model__cm, plot_title=f"[MC{args.mixed_class}] Weak_model", annot_fmt=",d")
    # plot_confusion_matrix(repaired_model__cm, plot_title=f"[MC{args.mixed_class}] Repaired_model")
    # plot_confusion_matrix(repaired_model__cm - weak_model__cm,
    #                       plot_title=f"[MC{args.mixed_class}] Repaired_model - Weak_model "
    #                                  f"[test_acc: {round(np.mean(test_acc) * 100, 2)}]", annot_fmt=",d")

    # strong_model__test_acc = model_evaluate(strong_model, test_loader, device=DEVICE, acc_in_percent=False)
    # print(weak_model__test_acc, strong_model__test_acc)
